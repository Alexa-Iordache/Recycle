...
let specialities = {
  getSpeciality(req, res, next) {
    mysql.query(`SELECT * FROM Medical_Specialities;`, (error, result) => {
      if (result.length == 0) {
        res.json({
          id: 1,
          error: "does not exist medical specialities",
          result: null,
        });
      }
  ...
  createSpecialitiesTable(req, res, next) {
    mysql.query(
      `SELECT S.MedicalSpecialityID, S.Name, S.Description, 
  ...
  getSpecialityName(req, res, next) {
    let name = req.body.params.name;

    mysql.query(
      `SELECT S.MedicalSpecialityID 
        FROM Medical_Specialities S
        WHERE S.Name = '${name}';`,
      (error, result) => {
        if (result.length == 0) {
          res.json({
            id: 1,
            error: "does not exist medical specialities with this name",
            result: null,
          });
        }
