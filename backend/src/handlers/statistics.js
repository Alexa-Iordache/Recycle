...
let statistics = {
    ...
    mysql.query(
      ...
      (error, result) => {
        if (result.length == 0) {
          res.json({
            id: 1,
            error: "does not exist any medical specialities",
            result: null,
          });
        }
  ...
  getNumberOfAppointmets(req, res, next) {
    mysql.query(
      `SELECT L.Street, L.StreetNumber, count(A.AppointmentID) as appointmetsNumber
      ...
      (error, result) => {
        if (result.length == 0) {
          res.json({
            id: 1,
            error: "does not exist any appointments in this location",
            result: null,
          });
        }
        res.json({ id: 1, error: null, result: result });
  ...
  getMostRecentDoctors(req, res, next) {
    mysql.query(
      `SELECT D.LastName, D.FirstName, S.Name, D.HiringDate 
        FROM Doctors D, Medical_Specialities S 
  ...
  getMostExpensiveAppointments(req, res, next) {
    mysql.query(
      `SELECT A.Appointment_code, A.Price, D.LastName, D.FirstName, S.Name 
      ...
      (error, result) => {
        if (result.length == 0) {
          res.json({
            id: 1,
            error: "does not exist any expensive appointments",
            result: null,
          });
        }
        res.json({ id: 1, error: null, result: result });
  ...
  getMostPatients(req, res, next) {
    mysql.query(
      `SELECT YEAR(P.BirthDate) as year, COUNT(P.BirthDate) as number
        FROM Patients P 
      ...
      (error, result) => {
        if (result.length == 0) {
          res.json({
            id: 1,
            error: "does not exist any patients born in this year",
            result: null,
          });
        }
        res.json({ id: 1, error: null, result: result });
  ...
  createAppointmentsList(req, res, next) {
    ...
    mysql.query(
      `SELECT A.Appointment_code, A.Price, L.Street, L.StreetNumber 
        FROM Appointments A, Locations L 
